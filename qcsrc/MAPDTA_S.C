...
static int sortcomp(const void *value1, const void *value2) {
            ...
            return memcmp(mi1->recKey, mi2->recKey, 47);
}
...
/*‚3. Zugriffschlüssel für die Daten, PIC X(47)                      */
               ...
               char *x_data, int *x_dataLen, int *x_lfdn, char *x_retcode)
{
  ...
  switch(x_action[0])
  {
    ...
    /*‚Ist noch genug Platz im Array? */
      ...
      MAPDTA_retcode = memcmp((*x_pgmStruct)->maxKeyVal, x_key, 47);
      if(MAPDTA_retcode < 0)
      {
        (*x_pgmStruct)->arrayCurrentSize++;
        memcpy((*x_pgmStruct)->dataArray[(*x_pgmStruct)->arrayCurrentSize - 1].recKey, x_key, 47);
        ...
        memcpy(&(*x_pgmStruct)->maxKeyVal, x_key, 47);
      }else
      {
        ...
        if(MAPDTA_retcode != 0)
        {
          memcpy(&MAPDTA_searchRec.recKey, x_key, 47);
        ...
        {
          memcpy(MAPDTA_searchResult->recKey, x_key, 47);
    ...
    case '3':
      ...
      if(memcmp(x_key, &(*x_pgmStruct)->maxKeyVal, 47) == 0)
      {
        ...
        else
        {
          ...
          memset(&(*x_pgmStruct)->maxKeyVal, 0, 47);
        }
        x_retcode[0] = ' ';
        break;
      }
    ...
    case '7':
      ...
      if (*x_lfdn >= 0 && *x_lfdn < (*x_pgmStruct)->arrayCurrentSize)
      {
        ...
        memcpy(x_key, (*x_pgmStruct)->dataArray[(*x_pgmStruct)->arrayCount].recKey, 47);
    ...
    case '8':
      ...
      if ((*x_pgmStruct)->arrayCount >= 0
       && (*x_pgmStruct)->arrayCount < (*x_pgmStruct)->arrayCurrentSize)
      {
        memcpy((*x_pgmStruct)->dataArray[(*x_pgmStruct)->arrayCount].recKey, x_key, 47);
